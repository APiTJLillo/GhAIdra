...
public class AnthropicProvider implements LLMProvider {
    ...
    public void configure(LLMConfig config) {
        ...
        this.config = (AnthropicConfig) config;
        setupClient();
    }

    private void setupClient() {
        if (!config.isValid()) {
            throw new IllegalStateException("Invalid Anthropic configuration");
        }
        ...
        return CompletableFuture.supplyAsync(() -> {
            ...
            while (retryCount < MAX_RETRIES) {
                try {
                    ...
                    Msg.debug(this, String.format("Making request to Anthropic API (model: %s)", config.getModel()));
                    ...
                    try (Response response = client.newCall(request).execute()) {
                        ...
                        String content = jsonResponse.path("content").path(0).path("text").asText();
